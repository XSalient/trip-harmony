...
export default function TripBudget() {
  ...
  const overBudgetMembers =
    summary?.memberBudgets?.filter((b: any) => b.overBudget) || [];
            ...
            <div className="grid grid-cols-2 gap-3">
              <Card className="bg-gradient-to-br from-primary/10 to-primary/5">
                <CardContent className="p-4">
                  ...
                  <p className="text-2xl font-bold">
                    {currency} {summary.total.toFixed(0)}
                  </p>
              ...
              <Card className="bg-gradient-to-br from-accent to-accent/50">
                <CardContent className="p-4">
                  ...
                  <p className="text-2xl font-bold">
                    {currency} {summary.perPerson.toFixed(0)}
                  </p>
                  <p className="text-xs text-muted-foreground mt-1">
                ...
                <CardContent className="p-3 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {overBudgetMembers.length} member
                      {overBudgetMembers.length > 1 ? "s" : ""} over their
                      budget limit
                    </p>
                  </div>
                </CardContent>
                ...
                <CardContent className="p-4">
                  ...
                  <div className="space-y-2">
                    {Object.entries(summary.byCategory).map(([cat, amt]) => {
                      ...
                      const pct =
                        summary.total > 0
                          ? ((amt as number) / summary.total) * 100
                          : 0;
                      return (
                        <div key={cat} className="space-y-1">
                          <div className="flex items-center justify-between text-xs">
                            ...
                            <span className="font-medium">
                              {currency} {(amt as number).toFixed(0)} (
                              {pct.toFixed(0)}%)
                            </span>
                          </div>
            ...
            {items.map((item: any) => {
              ...
              return (
                <Card key={item.id} className="border-border/50">
                  <CardContent className="p-3 flex items-center gap-3">
                    <div
                      className={`h-9 w-9 rounded-lg flex items-center justify-center shrink-0 ${categoryColors[item.category] || "bg-muted"}`}
                    >
