...
export default function TripVibeBoard() {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = trpc.vibeBoard.list.useQuery({ tripId }, { enabled: tripId > 0 });
  ...
  const handleVote = (vibeItemId: number, vote: "love" | "fine" | "veto") => {
    const currentVote = items
      ?.find((i: any) => i.id === vibeItemId)
    ...
    utils.vibeBoard.list.setData({ tripId }, (old: any) => {
      ...
      return old.map((i: any) => {
        ...
        const filtered =
          i.votes?.filter((v: any) => v.userId !== user?.id) || [];
        return {
          ...i,
          votes: isUnvote
            ? filtered
    ...
    if (isUnvote) {
      unvoteMutation.mutate(
        { vibeItemId },
        { onError: () => refetch(), onSuccess: () => refetch() }
      );
    } else {
      voteMutation.mutate(
        { vibeItemId, vote },
          ...
          <div className="space-y-3">
            {[...items]
              ...
              .map((item: any) => {
                ...
                const loves =
                  item.votes?.filter((v: any) => v.vote === "love").length || 0;
                const fines =
                  item.votes?.filter((v: any) => v.vote === "fine").length || 0;
                const vetos =
