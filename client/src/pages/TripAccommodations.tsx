...
export default function TripAccommodations() {
  ...
  const [refreshingMatch, setRefreshingMatch] = useState<
  ...
  const openEdit = (acc: any) => {
    ...
    setEditForm({
      ...
      pricePerNight: acc.pricePerNight
        ? String(parseFloat(acc.pricePerNight))
        : "",
    });
    setEditOpen(true);
  };
      ...
      if (result.success && result.data) {
        ...
        setForm(prev => ({
          ...
          amenities: d.amenities?.length
            ? [
                ...(prev.amenities
                  ? prev.amenities.split(",").map((s: string) => s.trim())
                  : []),
                ...d.amenities,
              ]
            ...
            <div className="flex items-center gap-2.5 p-3 rounded-xl bg-primary/5 border border-primary/20 cursor-pointer hover:bg-primary/10 transition-colors">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[11px] text-muted-foreground">
                  Help AI match accommodations to your must-haves, avoids, and
                  comfort needs
                </p>
              </div>
        ...
        {isLoading ? (
          ...
          <div className="space-y-4">
            {sorted.map((acc: any) => {
              ...
              const loves =
                acc.votes?.filter((v: any) => v.vote === "love").length || 0;
              const fines =
                        ...
                        {canManage && !acc.selected && (
                          <DropdownMenu>
                            ...
                            >
                              ...
                              <DropdownMenuItem
                                onClick={() => handleCloneIntoForm(acc)}
                                className="gap-2"
                              >
                    ...
                    <div className="flex flex-wrap gap-3 mb-3 text-xs">
                      {acc.singleBeds > 0 && (
                        <span className="flex items-center gap-1 text-muted-foreground">
                          <Bed className="h-3.5 w-3.5" /> {acc.singleBeds}{" "}
                          single
                        </span>
                      )}
                      {acc.doubleBeds > 0 && (
                        <span className="flex items-center gap-1 text-muted-foreground">
                          <Bed className="h-3.5 w-3.5" /> {acc.doubleBeds}{" "}
                          double
                        </span>
                      )}
                      {!acc.singleBeds &&
                        ...
                        acc.bedrooms > 0 && (
                          <span className="flex items-center gap-1 text-muted-foreground">
                            <Bed className="h-3.5 w-3.5" /> {acc.bedrooms} bed
                          </span>
                        )}
                      {acc.bathrooms > 0 && (
                        <span className="flex items-center gap-1 text-muted-foreground">
                          <Bath className="h-3.5 w-3.5" /> {acc.bathrooms} bath
                        </span>
                      )}
                      {acc.toilets > 0 && (
                        <span className="flex items-center gap-1 text-muted-foreground">
                          <Bath className="h-3.5 w-3.5" /> {acc.toilets} WC
                        </span>
                      )}
                      {acc.ensuites > 0 && (
                        <span className="flex items-center gap-1 text-muted-foreground">
                          <Bath className="h-3.5 w-3.5" /> {acc.ensuites}{" "}
                          ensuite
                        </span>
                      )}
                      {acc.freeParking && (
                    ...
                    <div className="flex items-baseline gap-3 mb-3">
                      {acc.pricePerNight && (
                        <div>
                          <span className="text-lg font-bold">
                            {currency}{" "}
                            {parseFloat(acc.pricePerNight).toFixed(0)}
                          </span>
                      ...
                      {acc.perPersonCost && (
                        <span className="text-sm text-muted-foreground">
                          ~{currency} {parseFloat(acc.perPersonCost).toFixed(0)}
                          /person total
                        </span>
                      )}
                    </div>
                                ...
                                >
                                  <div className="flex items-center gap-2">
                                    ...
                                    {matchStaleness(acc) === "stale" && (
                                      <span className="text-[10px] text-warning bg-warning-soft rounded-full px-1.5 py-0.5">
                                        May be out of date
                                      </span>
                                    )}
                                  </div>
      ...
      <Dialog open={editOpen && canContribute} onOpenChange={setEditOpen}>
        <DialogContent className="sm:max-w-sm">
          ...
          <div className="space-y-3 pt-1">
            ...
            <div>
              <Label className="text-xs">
                Price/night ({trip?.currency || "USD"}) (optional)
              </Label>
              <Input
                type="number"
                value={editForm.pricePerNight}
                onChange={e =>
                  setEditForm(p => ({ ...p, pricePerNight: e.target.value }))
                }
                className="rounded-lg mt-1"
              />
            </div>
