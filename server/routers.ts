...
async function runAccommodationMatchAnalysis(accommodationId: number, tripId: number): Promise<void> {
  try {
    ...
    const profileText = memberProfiles.map(p => {
      ...
      if (p.dna) {
        lines.push(`  Travel DNA — Budget: ${p.dna.budget}/10, Adventure: ${p.dna.adventure}/10, Comfort: ${p.dna.comfort}/10${p.dna.dietary ? `, Dietary: ${p.dna.dietary}` : ""}${p.dna.accessibility ? `, Accessibility: ${p.dna.accessibility}` : ""}`);
      } else {
...
Be honest and specific. Flag hard constraint failures (stairs, accessibility, dietary, etc.) clearly. If a member has no preferences set, give them a neutral score of 65.`;
    ...
    console.error("[matchAnalysis] failed for acc", accommodationId, err);
  }
}

/** Re-analyse all accommodations for a trip (call after preferences change). */
async function runTripMatchAnalyses(tripId: number): Promise<void> {
  try {
    ...
    console.error("[matchAnalysis] trip re-analyse failed", tripId, err);
  }
}
...
export const appRouter = router({
  ...
  auth: router({
    ...
    setPassword: protectedProcedure.input(z.object({
      ...
      // Only accounts that already have a password must prove the old one; magic-link
      // accounts have none to prove, and the session cookie is the authorisation there.
...
Output: [{"startDate":"2026-09-19","endDate":"2026-09-20","label":"Weekend Sep 19-20"},{"startDate":"2026-09-26","endDate":"2026-09-27","label":"Weekend Sep 26-27"}]`,
  ...
  accommodations: router({
    ...
    refreshMatch: protectedProcedure.input(z.object({
      accommodationId: z.number(),
      tripId: z.number(),
    })).mutation(async ({ input }) => {
